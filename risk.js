...
//   cause      대응하는 흔들림 계기 ('A' 시험후 자신감 / 'B' 레벨업후 부담 / 'C' 반 부적합)
...
const RISK_RULES = [
  ...
  // ─────────── 계기 B: 레벨업 직후 부담 ───────────
  {
    ...
    when: '레벨업 직후 난이도 상승을 "내가 못 따라간다"로 받아들이기 쉬운 성향입니다. 레벨업이 오히려 부담의 계기가 될 가능성이 있어요.',
    care: '레벨업 후 2~3주 완충 기간을 두면 좋습니다. 초반 과제 난이도를 한 단계 낮춰 시작하고, "지금은 적응 기간이라 어려운 게 당연하다"고 미리 안내해주세요.'
  },
  {
    id: 'B2',
    priority: 21,
    ...
    when: '확실히 아는 것 위주로 안정적으로 학습하는 성향입니다. 레벨업 후 낯선 표현·구문이 한꺼번에 많아지면 부담을 느낄 가능성이 있어요.',
...
const NO_RISK_MESSAGE =
  '현재 결과에서 특별히 살펴볼 부분은 나타나지 않았습니다. ' +
  '전반적으로 안정적인 성향이며, 아래 학습 성향을 참고해 강점을 살려주세요.';

// 결과지 상단에 항상 붙이는 성격 안내 (판정이 아니라 참고임을 명시)
const DISCLAIMER =
  '※ 아래 내용은 1회 검사 결과를 바탕으로 한 "가능성 참고"이며, ' +
